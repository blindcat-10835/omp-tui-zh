...
- 不要调用其他工具或恢复任务。
- 只做终端 `yield`：省略 `type`，在 `data` 中报告；或用 `type: string` 从最后一个 assistant 回合终结。
</system-reminder>
{{else}}
<system-reminder>
...
3. **产出成功** —— 仅在真正完成时：终端 `yield`；data 中单一最终结构化结果时省略 `type`；data 省略时用 `type: string` 从最后一个 assistant 回合终结。
